...
class CrossCamInference:
    ...
    def analyze(self, image_path: Path) -> Dict[str, Any]:
        ...
        if not inference:
            return {"findings": [], "detected_conditions": [], "overall_assessment": "未偵測到牙齒。", "aggregate_confidence": 0.0}
        ...
        for idx, (bbox, det_conf, cls_idx) in enumerate(zip(xyxy, confs, classes)):
            ...
            fdi_label = str(self._detector.names[cls_idx]) if isinstance(self._detector.names, (dict, list)) else str(cls_idx)
        ...
        if not patches:
            return {"findings": [], "detected_conditions": [], "overall_assessment": "未偵測到有效牙齒框。", "aggregate_confidence": 0.0}
        ...
        for info, prob in zip(meta, probs):
            if float(prob) < self.caries_threshold:
                continue
            severity = self._severity_from_probability(float(prob))
            findings.append(
                FindingResult(
                    tooth_label=f"FDI-{info['fdi_label']}",
                    bbox_xyxy=info["bbox"],
                    detector_conf=info["detector_conf"],
        ...
        if not findings:
            return {
                "findings": [],
                "detected_conditions": [],
                "overall_assessment": "未檢出疑似齲齒病灶。",
                "aggregate_confidence": float(probs.mean()) if len(probs) > 0 else 0.0,
            }

        aggregate_conf = float(np.mean([f.classifier_prob for f in findings]))
        ...
        summary = (
            f"偵測到 {len(findings)} 顆疑似齲齒 (平均信心 {aggregate_conf:.2f})"
        )
        ...
        serialized_findings = [
            {
                ...
                "region": {"bbox": finding.bbox_xywh, "mask_uri": None},
                "severity": finding.severity,
                "confidence": round(finding.classifier_prob, 4),
                "model_key": "cross_cam_caries",
                "model_version": "camAlignA",
                "extra": {
                    "detector_confidence": finding.detector_conf,
                    "classifier_probability": finding.classifier_prob,
                },
            }
